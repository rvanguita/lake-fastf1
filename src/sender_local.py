...
def find_delta_tables(root_path: str | Path) -> list[Path]:
    ...
    return [path.parent for path in root_path.rglob("_delta_log") if path.is_dir()]
...
def send_layer_to_mysql(
    ...
    try:
        for delta_path in find_delta_tables(root_path):
            ...
            table_name = f"{ID_TABLE}_{table_suffix}" if ID_TABLE else table_suffix

            dataframe = spark.read.format("delta").load(str(delta_path)).toPandas()
